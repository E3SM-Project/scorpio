            ...
            if(!fill)
            {
                ...
                if(!viobufs)
                {
                    return pio_err(ios, file, PIO_ENOMEM, __FILE__, __LINE__,
                                    "Writing variables (number of variables = %d) to file (%s, ncid=%d) failed. Unable to allocate %lld bytes to keep track of I/O buffer caches associated with the multiple variables", nvars, pio_get_fname_from_file(file), file->pio_ncid, (unsigned long long) (nvars * sizeof(viobuf_cache_t *)));
                }
            }
        }
#endif
                    ...
                    if (region)
                    {
                        ...
                        if(!fill)
                        {
                            if(!viobufs[nv])
                            {
                                ...
                                {
                                    return pio_err(ios, file, ierr, __FILE__, __LINE__,
                                                    "Writing variable %s (varid=%d, number of variables to write = %d) to file (%s, ncid=%d) failed. Internal error getting cached data assocaited with the variable %d (%s) for the PIO_IOTYPE_NETCDF4P iotype", pio_get_vname_from_file(file, varids[nv]), varids[nv], nvars, pio_get_fname_from_file(file), file->pio_ncid, nv, pio_get_vname_from_file(file, varids[nv]));
                                }
                            }
                ...
                if (regioncnt == num_regions - 1)
                {
                    ...
                    for (int nv = 0; nv < nvars; nv++)
                    {
                        ...
                        if(!fill)
                        {
                            ...
                            {
                                return pio_err(ios, file, ierr, __FILE__, __LINE__,
                                                "Writing variable %s (varid=%d, number of variables to write = %d) to file (%s, ncid=%d) using PIO_IOTYPE_PNETCDF iotype failed. Getting cached data for variable %d (%s) failed", pio_get_vname_from_file(file, varids[nv]), varids[nv], nvars, pio_get_fname_from_file(file), file->pio_ncid, nv, pio_get_vname_from_file(file, varids[nv]));
                            }
                            ...
                            {
                                return pio_err(ios, file, PIO_ENOMEM, __FILE__, __LINE__,
                                                "Writing fillvalues associated with variable %s (varid=%d, number of variables to write = %d) to file (%s, ncid=%d) using PIO_IOTYPE_PNETCDF iotype failed. Unable to allocate %lld bytes for dummy I/O buffer for writing the fillvalues for the variable %d (%s)", pio_get_vname_from_file(file, varids[nv]), varids[nv], nvars, pio_get_fname_from_file(file), file->pio_ncid, (unsigned long long) (sizeof(viobuf_cache_t)), nv, pio_get_vname_from_file(file, varids[nv]));
                            }
                              ...
                              io_desc_t *iodesc, int fill, const int *frame)
{
    ...
    if (ios->ioproc)
    {
         ...
         * Since data corresponding to multiple io decompositions are cached
         * at the same time, copy data for all variables into a single
         * buffer corresponding to the io decomposition (ioid),
         * file->iobuf[iodesc->ioid - PIO_IODESC_START_ID]
         */
        if(!fill)
        {
            /* FIXME : We only need iodesc->mpitype_size * llen * nvars */
            size_t file_iobuf_sz = iodesc->mpitype_size * iodesc->maxiobuflen * nvars;
            file->iobuf[iodesc->ioid - PIO_IODESC_START_ID] = bget(file_iobuf_sz);
            if(!file->iobuf[iodesc->ioid - PIO_IODESC_START_ID])
            {
                return pio_err(ios, file, PIO_ENOMEM, __FILE__, __LINE__,
                                "Writing multiple variables (number of variables = %d) to file (%s, ncid=%d) using serial I/O failed. Unable to allocate(BGET) %lld bytes for a single buffer to compact data for multiple variables pending to be written to the file", nvars, pio_get_fname_from_file(file), file->pio_ncid, (unsigned long long) file_iobuf_sz);
            }
            ierr = pio_file_compact_and_copy_rearr_data(
                      file->iobuf[iodesc->ioid - PIO_IODESC_START_ID],
                      file_iobuf_sz, iodesc, file, varids, frame, nvars);
            if(ierr != PIO_NOERR)
            {
                return pio_err(ios, file, ierr, __FILE__, __LINE__,
                                "Writing multiple variables (number of variables = %d) to file (%s, ncid=%d) using serial I/O failed. Internal error compacting rearranged data for multiple variables into a single buffer", nvars, pio_get_fname_from_file(file), file->pio_ncid);
            }
        }
...
int flush_output_buffer(file_desc_t *file, bool force, PIO_Offset addsize)
{
    ...
    if (ierr != NC_ENULLABUF)
    {
        return pio_err(NULL, file, PIO_EBADID, __FILE__, __LINE__,
                          "Internal error flushing data written (ensuring/waiting_for all pending data is written to disk) to file (%s, ncid=%d). Unable to query the PnetCDF library buffer usage for the file", file->fname, file->pio_ncid);
    }
