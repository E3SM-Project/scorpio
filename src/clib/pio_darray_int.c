    ...
    if (!CN_bpool)
    {
        if (!(CN_bpool = malloc(pio_cnbuffer_limit)))
        {
            return pio_err(ios, NULL, PIO_ENOMEM, __FILE__, __LINE__,
                            "Internal error allocating buffer for caching user data (write data) in the compute processes. Out of memory allocating %lld bytes", (long long int) pio_cnbuffer_limit);
        }
        ...
        {
            return pio_err(ios, NULL, PIO_ENOMEM, __FILE__, __LINE__,
                            "Internal error allocating buffer for caching user data (write data) in the compute processes. Unable to initialize the buffer pool with the cache buffer");
        }
