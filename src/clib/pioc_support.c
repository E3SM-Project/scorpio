                 ...
                 int mode, int retry)
{
    ...
    {
        return pio_err(ios, NULL, ierr, __FILE__, __LINE__,
                        "Opening file (%s) failed. Although the file was opened successfully, querying the number of variables in the file failed", (filename) ? filename : "UNKNOWN");
    }
    ...
    {
        return pio_err(ios, NULL, ierr, __FILE__, __LINE__,
                        "Opening file (%s) failed. Although the file was opened successfully, querying the number of unlimited dimensions in the file failed", (filename) ? filename : "UNKNOWN");
    }
    ...
    {
        return pio_err(ios, NULL, ierr, __FILE__, __LINE__,
                        "Opening file (%s) failed. Although the file was opened successfully, querying the unlimited dimension ids (number of unlimited dimensions = %d) in the file failed", (filename) ? filename : "UNKNOWN", nunlimdim);
    }
    ...
    {
        return pio_err(ios, NULL, ierr, __FILE__, __LINE__,
                        "Openfile file (%s) failed. Unexpected internal error, although the file was opened successfully encountered error querying for corresponding internal structures (file). Invalid file id (ncid=%d)", (filename) ? filename : "UNKNOWN", *ncidp);
    }
    ...
    for (int v = 0; v < nvars; v++)
    {
        ...
        {
            const char *vname = (file && (v >= 0) && (v < PIO_MAX_VARS)) ? file->varlist[v].vname : ((v == PIO_GLOBAL) ? "PIO_GLOBAL" : "UNKNOWN");
            return pio_err(ios, NULL, ierr, __FILE__, __LINE__,
                            "Opening file (%s) failed. Although opening the file was successful, querying the number of dimensions for variable %s (varid=%d) failed", (filename) ? filename : "UNKNOWN", vname, v);
        }
        ...
        if (var_ndims)
        {
            ...
            if ((ierr = PIOc_inq_vardimid(*ncidp, v, var_dimids)))
            {
                const char *vname = (file && (v >= 0) && (v < PIO_MAX_VARS)) ? file->varlist[v].vname : ((v == PIO_GLOBAL) ? "PIO_GLOBAL" : "UNKNOWN");
                return pio_err(ios, NULL, ierr, __FILE__, __LINE__,
                                "Opening file (%s) failed. Although opening the file was successful, querying the dimension ids (number of dimensions = %d) for variable %s (varid=%d) failed", (filename) ? filename : "UNKNOWN", var_ndims, vname, v);
            }
        }
        ...
        if ((ierr = add_to_varlist(v, rec_var, &file->varlist2)))
        {
            const char *vname = (file && (v >= 0) && (v < PIO_MAX_VARS)) ? file->varlist[v].vname : ((v == PIO_GLOBAL) ? "PIO_GLOBAL" : "UNKNOWN");
            return pio_err(ios, NULL, ierr, __FILE__, __LINE__,
                            "Opening file (%s) failed. Although opening the file was successful, internal error adding variable %s (varid=%d) to the file variable list", (filename) ? filename : "UNKNOWN", vname, v);
        }
    }
                        ...
                        int max_pend_req_i2c)
{
    ...
    {
        return pio_err(NULL, NULL, PIO_EINVAL, __FILE__, __LINE__,
                        "Setting rearranger options failed. iosysid=%d, fcd=%d. Invalid arguments provided, comm_type is %s (expected PIO_REARR_COMM_P2P or PIO_REARR_COMM_FC_1D_COMP2IO), fcd = %d (expected > 0 and <= %d), max_pend_req_c2i = %d (expected >= 0 or PIO_REARR_COMM_UNLIMITED_PEND_REQ=%d), max_pend_req_i2c = %d (expected >=0 or PIO_REARR_COMM_UNLIMITED_PEND_REQ=%d)", iosysid, fcd, (comm_type == PIO_REARR_COMM_P2P) ? "PIO_REARR_COMM_P2P" : ((comm_type == PIO_REARR_COMM_FC_1D_COMP2IO) ? "PIO_REARR_COMM_FC_1D_COMP2IO" : ((comm_type == PIO_REARR_COMM_FC_1D_IO2COMP) ? "PIO_REARR_COMM_FC_1D_IO2COMP" : "UNKNOWN")), fcd, PIO_REARR_COMM_FC_2D_DISABLE, max_pend_req_c2i, PIO_REARR_COMM_UNLIMITED_PEND_REQ, max_pend_req_i2c, PIO_REARR_COMM_UNLIMITED_PEND_REQ);
    }
