...
static inline std::string get_trace_mdata_header(int iosysid, int mpi_wrank)
{
  ...
  std::string decomp_info = std::string("I/O Decomp file : ") + SPIO_Util::Tracer::get_trace_decomp_fname(iosysid, mpi_wrank) + "\n";

  std::string hdr = banner + spio_trace_mdata_info + iosys_info + rank_info + log_info + decomp_info + banner;
...
SPIO_Util::Logger::MPI_logger<std::ofstream> &SPIO_Util::Tracer::get_iosys_trace_mdata_logger(int iosysid, int mpi_wrank)
{
  ...
  if(iter == SPIO_Util::Tracer::GVars::trace_mdata_loggers_.end()){
    ...
    std::pair<std::map<std::string, SPIO_Util::Logger::MPI_logger<std::ofstream> >::iterator, bool> res = SPIO_Util::Tracer::GVars::trace_mdata_loggers_.insert({std::to_string(iosysid), lstr});
    assert(res.second);
    iter = res.first;
  }
  return iter->second;
