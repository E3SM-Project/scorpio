...
int PIOc_inq(int ncid, int *ndimsp, int *nvarsp, int *ngattsp, int *unlimdimidp)
{
    ...
    if ((ierr = pio_get_file(ncid, &file)))
    {
        return pio_err(NULL, NULL, ierr, __FILE__, __LINE__,
                        "Inquiring information about file (ncid=%d) failed. Unable to find internal structure assocaited with the file id", ncid);
    }
    ios = file->iosystem;
...
int PIOc_inq_unlimdims(int ncid, int *nunlimdimsp, int *unlimdimidsp)
{
    ...
    {
        return pio_err(NULL, NULL, ierr, __FILE__, __LINE__,
                        "Inquiring unlimited dimension information failed on file (ncid=%d). Unable to find internal structure associated with the file id", ncid);
    }
    ios = file->iosystem;
...
int PIOc_inq_type(int ncid, nc_type xtype, char *name, PIO_Offset *sizep)
{
    ...
    {
        return pio_err(NULL, NULL, ierr, __FILE__, __LINE__,
                        "Inquiring type information failed on file (ncid=%d). Unable to find internal structure associated with the file id", ncid);
    }
    ios = file->iosystem;
...
int PIOc_inq_format(int ncid, int *formatp)
{
    ...
    {
        return pio_err(NULL, NULL, ierr, __FILE__, __LINE__,
                        "Inquiring format failed on file (ncid=%d). Unable to find internal structure associated with the file id", ncid);
    }
    ios = file->iosystem;
...
int PIOc_inq_dim(int ncid, int dimid, char *name, PIO_Offset *lenp)
{
    ...
    {
        return pio_err(NULL, NULL, ierr, __FILE__, __LINE__,
                        "Inquiring dimension (dimid=%d) information failed on file (ncid=%d). Unable to find internal structure associated with the file id", dimid, ncid);
    }
    ios = file->iosystem;
...
int PIOc_inq_dimid(int ncid, const char *name, int *idp)
{
    ...
    if ((ierr = pio_get_file(ncid, &file)))
    {
        const char *dname = (name) ? name : "UNKNOWN";
        return pio_err(NULL, NULL, ierr, __FILE__, __LINE__,
                        "Inquiring id of dimension %s failed on file (ncid=%d). Unable to find internal structure associated with the file id", dname, ncid);
    }
    ios = file->iosystem;
                 ...
                 int *dimidsp, int *nattsp)
{
    ...
    {
        return pio_err(NULL, NULL, ierr, __FILE__, __LINE__,
                        "Inquiring information on variable (varid=%d) failed on file (ncid=%d). Unable to find internal structure associated with the file id", varid, ncid);
    }
    ios = file->iosystem;
...
int PIOc_inq_varid(int ncid, const char *name, int *varidp)
{
    ...
    if ((ierr = pio_get_file(ncid, &file)))
    {
        ...
        return pio_err(NULL, NULL, ierr, __FILE__, __LINE__,
                        "Inquiring id for variable %s failed on file (ncid=%d). Unable to find internal structure associated with the file id", vname, ncid);
    }
    ios = file->iosystem;
    ...
    if (file->iotype == PIO_IOTYPE_ADIOS)
    {
        ...
        *varidp = file->num_vars;
        ...
        /* Some codes moved from pio_darray.c */
        {
            ...
            {
                adios_define_attribute_byvalue(file->adios_group, "__pio__/ndims", av->name,
                                               adios_integer, 1, &av->ndims);
                adios_define_attribute_byvalue(file->adios_group, "__pio__/nctype", av->name,
