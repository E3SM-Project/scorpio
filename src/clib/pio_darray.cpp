                            ...
                            const void **fillvalue, bool flushtodisk)
{
  ...
  for(int i=0; i<nvars; i++){
    ...
    ierr = mtimer_pause(file->varlist[varids[i]].wr_mtimer, &(var_mtimer_was_running[i]));
    if(ierr != PIO_NOERR){
      return pio_err(ios, file, ierr, __FILE__, __LINE__,
  ...
  ierr = mtimer_get_wtime(file->varlist[varids[0]].wr_rearr_mtimer, &rearr_time);
  if(ierr != PIO_NOERR){
    return pio_err(ios, file, ierr, __FILE__, __LINE__,
  ...
  for(int i=0; i<nvars; i++)
  {
    ...
    ierr = mtimer_update(file->varlist[varids[i]].wr_rearr_mtimer, rearr_time);
    if(ierr != PIO_NOERR){
     ...
     */
    ierr = mtimer_update(file->varlist[varids[i]].wr_mtimer, rearr_time);
    if(ierr != PIO_NOERR){
                      ...
                      const void *fillvalue)
{
  ...
  if(iodesc->needsfill){
    ...
    if(fillvalue){
      ...
      LOG((3, "copied user-provided fill value iodesc->mpitype_size = %d", iodesc->mpitype_size));
    }
    else{
      void *fill;
