...
static int PIOc_write_decomp_adios(file_desc_t *file, int ioid)
{
        ...
        if (file->adios_io_process == 1 && file->block_myrank == 0)
        {
            ...
            if (file->iosystem->adios_lossy_compression_method == ADIOS_COMPRESSION_METHOD_ZFP)
            {
                if (count_val == 1)
                {
                    ...
                    printf("PIO: WARNING: ZFP does not support 0D data, compression is skipped to write variable %s to file %s\n",
                           name, pio_get_fname_from_file(file));
                }
            }
            #endif
    ...
    if (file->adios_io_process == 1 && file->block_myrank == 0)
    {
        ...
        printf("PIO: WARNING: ZFP does not support 0D data, compression is skipped to write variable %s to file %s\n",
               name_varid, pio_get_fname_from_file(file));
        ...
        if (av->num_wb_cnt >= av->max_buffer_cnt)
        {
            ...
            if (file->iosystem->adios_lossy_compression_method == ADIOS_COMPRESSION_METHOD_ZFP)
            {
                if (count_val == 1)
                {
                    ...
                    printf("PIO: WARNING: ZFP does not support 0D data, compression is skipped to write variable num_data_block_writers/%s to file %s\n",
                           av->name, pio_get_fname_from_file(file));
                }
            }
            #endif
        ...
        if (av->fillval_cnt >= av->max_buffer_cnt)
        {
            ...
            if (file->iosystem->adios_lossy_compression_method == ADIOS_COMPRESSION_METHOD_ZFP)
            {
                if (count_val == 1)
                {
                    ...
                    printf("PIO: WARNING: ZFP does not support 0D data, compression is skipped to write variable fillval_id/%s to file %s\n",
                           av->name, pio_get_fname_from_file(file));
                }
            }
            #endif
        ...
        if (av->decomp_cnt >= av->max_buffer_cnt)
        {
            ...
            if (file->iosystem->adios_lossy_compression_method == ADIOS_COMPRESSION_METHOD_ZFP)
            {
                if (count_val == 1)
                {
                    ...
                    printf("PIO: WARNING: ZFP does not support 0D data, compression is skipped to write variable decomp_id/%s to file %s\n",
                           av->name, pio_get_fname_from_file(file));
                }
            }
            #endif
        ...
        if (av->frame_cnt >= av->max_buffer_cnt)
        {
            ...
            if (file->iosystem->adios_lossy_compression_method == ADIOS_COMPRESSION_METHOD_ZFP)
            {
                if (count_val == 1)
                {
                    ...
                    printf("PIO: WARNING: ZFP does not support 0D data, compression is skipped to write variable frame_id/%s to file %s\n",
                           av->name, pio_get_fname_from_file(file));
                }
            }
            #endif
