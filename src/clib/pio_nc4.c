                         ...
                         int *deflate_levelp)
{
    ...
    if (shufflep)
        if ((mpierr = MPI_Bcast_Wrapper(shufflep, 1, MPI_INT, ios->ioroot, ios->my_comm)))
        {
            spio_ltimer_stop(ios->io_fstats->tot_timer_name);
            spio_ltimer_stop(file->io_fstats->tot_timer_name);
    ...
    if (deflatep)
        if ((mpierr = MPI_Bcast_Wrapper(deflatep, 1, MPI_INT, ios->ioroot, ios->my_comm)))
        {
            spio_ltimer_stop(ios->io_fstats->tot_timer_name);
            spio_ltimer_stop(file->io_fstats->tot_timer_name);
            return check_mpi(NULL, file, mpierr, __FILE__, __LINE__);
    ...
    if (deflate_levelp)
        if ((mpierr = MPI_Bcast_Wrapper(deflate_levelp, 1, MPI_INT, ios->ioroot, ios->my_comm)))
        {
            spio_ltimer_stop(ios->io_fstats->tot_timer_name);
            spio_ltimer_stop(file->io_fstats->tot_timer_name);
            return check_mpi(NULL, file, mpierr, __FILE__, __LINE__);
...
int PIOc_def_var_chunking(int ncid, int varid, int storage, const PIO_Offset *chunksizesp)
{
    ...
    if (ios->async)
    {
        ...
        if ((mpierr = MPI_Bcast_Wrapper(&ndims, 1, MPI_INT, ios->comproot, ios->my_comm)))
        {
            spio_ltimer_stop(ios->io_fstats->tot_timer_name);
            spio_ltimer_stop(file->io_fstats->tot_timer_name);
            return check_mpi(NULL, file, mpierr, __FILE__, __LINE__);
...
int PIOc_inq_var_chunking(int ncid, int varid, int *storagep, PIO_Offset *chunksizesp)
{
    ...
    if (storagep)
        if ((mpierr = MPI_Bcast_Wrapper(storagep, 1, MPI_INT, ios->ioroot, ios->my_comm)))
        {
            spio_ltimer_stop(ios->io_fstats->tot_timer_name);
            spio_ltimer_stop(file->io_fstats->tot_timer_name);
            return check_mpi(NULL, file, mpierr, __FILE__, __LINE__);
    ...
    if (chunksizesp)
        if ((mpierr = MPI_Bcast_Wrapper(chunksizesp, ndims, MPI_OFFSET, ios->ioroot, ios->my_comm)))
        {
            spio_ltimer_stop(ios->io_fstats->tot_timer_name);
            spio_ltimer_stop(file->io_fstats->tot_timer_name);
            return check_mpi(NULL, file, mpierr, __FILE__, __LINE__);
...
int PIOc_inq_var_endian(int ncid, int varid, int *endianp)
{
    ...
    /* Broadcast results to all tasks. */
    if (endianp)
        if ((mpierr = MPI_Bcast_Wrapper(endianp, 1, MPI_INT, ios->ioroot, ios->my_comm)))
        {
            spio_ltimer_stop(ios->io_fstats->tot_timer_name);
            spio_ltimer_stop(file->io_fstats->tot_timer_name);
            return check_mpi(NULL, file, mpierr, __FILE__, __LINE__);
                         ...
                         float *preemptionp)
{
    ...
    if (nelemsp)
    {
        if ((mpierr = MPI_Bcast_Wrapper(nelemsp, 1, MPI_OFFSET, ios->ioroot, ios->my_comm)))
        {
            spio_ltimer_stop(ios->io_fstats->tot_timer_name);
            return check_mpi(ios, NULL, mpierr, __FILE__, __LINE__);
        }
    ...
    if (preemptionp)
    {
        if ((mpierr = MPI_Bcast_Wrapper(preemptionp, 1, MPI_FLOAT, ios->ioroot, ios->my_comm)))
        {
            spio_ltimer_stop(ios->io_fstats->tot_timer_name);
            return check_mpi(ios, NULL, mpierr, __FILE__, __LINE__);
        }
