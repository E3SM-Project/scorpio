...
! only pio_offset is intended for export from kinds
  ...
  use piolib_mod, only : pio_initdecomp, &
       ...
       pio_set_rearr_opts, pio_set_fill, pio_get_version_major, pio_get_version_minor, &
       pio_get_version_patch, pio_get_version_string, pio_get_version_hash, &
       pio_get_version
  ...
  use pio_types, only : io_desc_t, file_desc_t, var_desc_t, iosystem_desc_t, &
       ...
       pio_rearr_subset, pio_rearr_box, pio_rearr_any, pio_rearr_contig, &
