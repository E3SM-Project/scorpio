...
#define SC_ARRAY_LEN 3
    ...
    int ntasks;   /* Number of tasks in test_comm. */
    int mpierr;   /* Return value from MPI calls. */
    PIO_Offset array1[SC_ARRAY_LEN] = {7, 42, 14};
    PIO_Offset array2[SC_ARRAY_LEN] = {2, 3, 7};
    PIO_Offset array3[SC_ARRAY_LEN] = {90, 180, 270};
    PIO_Offset array4[SC_ARRAY_LEN] = {1, 180, 270};
    ...
    if (lgcd_array(SC_ARRAY_LEN, array1) != 7)
        return ERR_WRONG;
    if (lgcd_array(SC_ARRAY_LEN, array2) != 1)
        return ERR_WRONG;
    if (lgcd_array(SC_ARRAY_LEN, array3) != 90)
        return ERR_WRONG;
    if (lgcd_array(SC_ARRAY_LEN, array4) != 1)
        return ERR_WRONG;
...
int test_lists()
{
    ...
    if (pio_delete_iosystem_from_list(42) == PIO_NOERR)
        return ERR_WRONG;
    if (pio_delete_file_from_list(42) == PIO_NOERR)
        return ERR_WRONG;
