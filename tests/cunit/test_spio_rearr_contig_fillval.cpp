...
int test_c2i_block_data_exp_fillval(MPI_Comm comm, int wrank, int wsz)
{
  int ret = PIO_NOERR;
  ...
  try{
    ...
    const std::size_t FILLVAL_MASK_OFFSET = 3;
    const double FILLVAL = 123.456;
    const PIO_Offset PIO_COMPMAP_FILLVAL = -1;
    ...
    if(ios->ioproc){
      ...
      std::fill(rdata.begin(), rdata.end(), FILLVAL);

      exp_data.resize(rdata_sz);
      std::iota(exp_data.begin(), exp_data.end(), ios->io_rank * rearr_iochunk_sz);

      for(std::size_t i = 0, gstart = ios->io_rank * rearr_iochunk_sz; i < exp_data.size(); i++){
        if((gstart + i) % FILLVAL_MASK_OFFSET == 0){
          exp_data[i] = FILLVAL;
        }
      }
    }
    ...
    std::transform(compmap.cbegin(), compmap.cend(), compmap.begin(),
      [FILLVAL_MASK_OFFSET](const PIO_Offset i){ return (i % FILLVAL_MASK_OFFSET == 0) ? PIO_COMPMAP_FILLVAL : i; });
    std::transform(compmap.cbegin(), compmap.cend(), sdata.begin(),
      [PIO_COMPMAP_FILLVAL, FILLVAL](const PIO_Offset i){ return static_cast<double>((i != PIO_COMPMAP_FILLVAL) ? i : FILLVAL); });
...
int test_c2i_block_data_imp_fillval(MPI_Comm comm, int wrank, int wsz)
{
  int ret = PIO_NOERR;
  int nio_procs = (wsz/2 > 0) ? wsz/2 : wsz;
  iosystem_desc_t *ios = get_iosystem(comm, wrank, wsz, nio_procs);
  ...
  try{
    ...
    const std::size_t wrank_compmap_end_idx = static_cast<std::size_t>(wrank * LOCAL_COMPMAP_SZ + LOCAL_COMPMAP_SZ);
    for(std::size_t i = wrank * LOCAL_COMPMAP_SZ; i < wrank_compmap_end_idx; i++){
      if(i % FILLVAL_MASK_OFFSET != 0){
        compmap.push_back(i);
        sdata.push_back(static_cast<double>(i));
      }
    }
...
int test_block_data_exp_fillval(MPI_Comm comm, int wrank, int wsz)
{
  int ret = PIO_NOERR;
  int nio_procs = (wsz/2 > 0) ? wsz/2 : wsz;
  iosystem_desc_t *ios = get_iosystem(comm, wrank, wsz, nio_procs);
  ...
  try{
    ...
    std::fill(sdata.begin(), sdata.end(), FILLVAL);
...
int test_block_data_imp_fillval(MPI_Comm comm, int wrank, int wsz)
{
  int ret = PIO_NOERR;
  int nio_procs = (wsz/2 > 0) ? wsz/2 : wsz;
  iosystem_desc_t *ios = get_iosystem(comm, wrank, wsz, nio_procs);
...
int test_driver(MPI_Comm comm, int wrank, int wsz, int *num_errors)
{
  ...
  std::vector<std::pair<std::string, std::function<int(MPI_Comm, int, int)> > > test_funcs = {
      {"test_c2i_block_data_exp_fillval", test_c2i_block_data_exp_fillval},
      {"test_c2i_block_data_imp_fillval", test_c2i_block_data_imp_fillval},
      {"test_block_data_exp_fillval", test_block_data_exp_fillval},
      {"test_block_data_imp_fillval", test_block_data_imp_fillval},
