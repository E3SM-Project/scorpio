...
#include <mpif.h>
  ...
  call MPI_Init_thread(MPI_THREAD_MULTIPLE, ts, ierr)
  if((ierr /= MPI_SUCCESS) .or. (ts /= MPI_THREAD_MULTIPLE)) then
    print *, "MPI_THREAD_MULTIPLE required, but not available"
    stop 99
  end if
...
contains
  ...
  subroutine pio_type2typename(pio_type, pio_typename)
    integer, intent(in) :: pio_type
    character(len=*), intent(out) :: pio_typename

    pio_typename = "unknown"
    if(pio_type == PIO_IOTYPE_NETCDF) then
      pio_typename = "netcdf"
    else if(pio_type == PIO_IOTYPE_NETCDF4P) then
      pio_typename = "netcdf4p"
    else if(pio_type == PIO_IOTYPE_NETCDF4C) then
      pio_typename = "netcdf4c"
    else if(pio_type == PIO_IOTYPE_PNETCDF) then
      pio_typename = "pnetcdf"
    else
      !print *, "ERROR: Unrecognized pio type :", pio_typename,&
      !          __FILE__, __LINE__
    endif
  end subroutine pio_type2typename
  ...
  subroutine compute_phase(rank, start_idx)
    integer, intent(in) :: rank
    integer, intent(in) :: start_idx

    ! Some computation to take up CPU time
    integer, parameter :: NITERS = 100
    integer, parameter :: M = 50, N = 50, P = 50
  ...
  subroutine pioperformance_rearrtest(filename, piotypes, mype, npe_base, &
    ...
    character(len=PIO_MAX_NAME) :: varname, typename
    integer, parameter :: MAX_TIMESTAMPS = 2
    double precision :: wall(MAX_TIMESTAMPS), sys(MAX_TIMESTAMPS),&
                        usr(MAX_TIMESTAMPS)
    double precision :: walltime_iophase, walltime_compphase
