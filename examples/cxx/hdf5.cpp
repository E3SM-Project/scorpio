...
int test_hdf5_2d_non_dist_vars_slice(MPI_Comm comm, int comm_rank, int comm_sz, hid_t fh, hid_t dcpid)
{
  int ret = 0;
  herr_t hret = 0;
  ...
  hid_t varid_non_dist = H5Dcreate2(fh, "tmp_var_2d_non_dist_sliced", H5T_NATIVE_FLOAT, fsid, H5P_DEFAULT, dcpid, H5P_DEFAULT); check_err(comm_rank, (varid_non_dist != H5I_INVALID_HID) ? 0 : -1, "Defining/Creating variable (not distributed) failed", __LINE__);
  ...
  for(int ilev = 0; ilev < DIM_LEV_SZ; ilev++){
    ...
    if(comm_rank == 0){
      nelems = DIM_NCOLS_SZ_PER_PROC * comm_sz;
    }
    else{
      nelems = 0;
    }
    ...
    msid = H5Screate_simple(1, &nelems, NULL); check_err(comm_rank, (msid != H5I_INVALID_HID) ? 0 : -1, "Creating memory space for write data failed", __LINE__);
    hret = H5Dwrite(varid_non_dist, H5T_NATIVE_FLOAT, msid, fsid, dxpid, &(var_non_dist[ilev][0])); check_err(comm_rank, hret, "Writing data to variable failed", __LINE__);
...
int test_hdf5_2d_vars(MPI_Comm comm, int comm_rank, int comm_sz, hid_t fh, hid_t dcpid)
{
  ...
  ret = test_hdf5_2d_non_dist_vars_slice(comm, comm_rank, comm_sz, fh, dcpid); check_err(comm_rank, ret, "Writing 2D non-distributed variable, one slice at a time, failed", __LINE__);
